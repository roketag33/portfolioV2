...
import { TIMELINE_IDS } from '@/data/experience'
...
export default function AboutPage() {
    ...
    const timelineIds = TIMELINE_IDS
    const bobyIndex = timelineIds.indexOf('boby')
                ...
                <div className="mb-40" ref={timelineRef}>
                    ...
                    <div className="relative max-w-5xl mx-auto">
                        ...
                        {timelineIds.map((id, index) => {
                            const year = tExp(`${id}.year`)
                            const role = tExp(`${id}.role`)
                            const company = tExp(`${id}.company`)
                            const desc = tExp(`${id}.desc`)
                            const stack = tExp(`${id}.stack`).split(', ')
                            const schoolName = tExp(`${id}.school_name`)
                            const schoolDegree = tExp(`${id}.school_degree`)
                            const schoolDesc = tExp(`${id}.school_desc`)
                            const schoolStatus = tExp(`${id}.school_status`)

                            return (
                                <SpotlightCard key={id} className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-8 md:gap-24 items-center mb-16 md:mb-24 last:mb-0 group rounded-3xl p-6 md:p-0 bg-white/[0.02] md:bg-transparent border border-white/5 md:border-none">
                                    ...
                                    >
                                        <div className="flex flex-col md:flex-row-reverse md:items-baseline gap-2 mb-2 justify-end">
                                            <h3 className="text-2xl font-bold text-white group-hover:text-purple-400 transition-colors duration-300">{role}</h3>
                                            <span className="text-sm font-mono text-neutral-400 whitespace-nowrap">@ {company}</span>
                                        </div>
                                    ...
                                    <div className="hidden md:flex flex-col items-center justify-center relative z-20 h-full">
                                        ...
                                        <div
                                            ...
                                            style={{
                                                top: '50%',
                                                bottom: index === timelineIds.length - 1 ? '50%' : '-3rem'
                                            }}
                                        />
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col gap-1 mb-2">
                                            <h3 className="text-xl font-bold text-white group-hover:text-purple-400 transition-colors">{schoolName}</h3>
                                            <span className="text-sm font-mono text-purple-300">{schoolDegree}</span>
                                        </div>
